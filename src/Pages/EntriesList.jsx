...
const EntriesList = () => {
  ...
  const fetchEntries = async () => {
    try {
      ...
      toast.error("Failed to fetch entries. Check backend connection.");
    }
  };
  ...
  const handleUpdate = async () => {
    try {
      const updatedPayload = {
        ...editingEntry,
        sprayingArea: parseFloat(editingEntry.sprayingArea) || 0,
        expense: parseFloat(editingEntry.expense) || 0,
        profit: parseFloat(editingEntry.profit) || 0,
      };
      await axios.put(`${API_BASE_URL}/${editingEntry.id}`, updatedPayload, {
  ...
  const applyDateFilter = (entries) => {
    ...
    return entries.filter((entry) => {
      ...
      const entryDate = new Date(`${year}-${month}-${day}`); // Parse dd-MM-yyyy
   ...
   * =============================== */
  const totalProfit = filteredEntries.reduce((acc, entry) => acc + (entry.profit || 0), 0);
  const totalExpense = filteredEntries.reduce((acc, entry) => acc + (entry.expense || 0), 0);
